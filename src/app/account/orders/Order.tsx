  ...
  return (
    <div className="p-3 rounded-lg border space-y-5">
      <div className="flex gap-2 justify-between">
        ...
        <div className="text-sm flex flex-wrap items-center h-max gap-1 justify-end"><span>оплачено</span><span className="text-base font-semibold text-nowrap">{total} ₽</span></div>
      </div>
        ...
        <ul className="overflow-x-auto flex">
          {
            order.items.map(item =>
              <li key={item.product.id} title="Перейти на страницу товара" className="shrink-0 p-3 transition-all border-2 border-transparent hover:border-gray-100 rounded-lg">
